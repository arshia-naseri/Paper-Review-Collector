...
focussed on the linear case, which is known as Independent Component Analysis (ICA; Hyv¨arinen
...
ICA models is both ill-deﬁned and still far from being solved.
...
We show how the GAN framework allows us to deﬁne new objectives for learning statistically in-
...
dent of the mixing function and doesn’t use properties that are speciﬁc for the linear or so-called
...
if all the variables are mutually independent. One beneﬁt of working with MI as a measure of de-
...
in the same space as the samples of the data set. The discriminator is a classiﬁer D(·) which is
...
GAN framework, the discriminator is optimized using the cross-entropy loss. The full deﬁnition of
...
large as the number of sources, linear ICA is guaranteed to be identiﬁable (Comon, 1994) up to a
permutation and rescaling of the sources. When the mixing of the signals is not linear, identiﬁability
cannot be guaranteed in general and the problem is ill-posed. However, under certain circumstances,
some speciﬁc types of non-linear mixtures like post non-linear mixtures (PNL) can still be separated
...
the product of the marginals (cid:81)
i p(zi). We now want to measure how dependent/independent the
...
Ez∼q(z)[f (z)] − Ez∼r(z)[f (z)].

(4)

If we substitute q and r for p(z) and (cid:81)
i p(zi), respectively, we can consider Equation 4 to be an ap-
...
To obtain samples from the complete product (cid:81)M
i=1 p(zi), the same method can be used by taking
...
not be truly independent of the ﬁrst. We argue that this is not a big problem in practice as long as
...
torized distribution without imposing a speciﬁc prior. While it may be more difﬁcult to learn the
...
Figure 1: Schematic representation of the entire system for learning non-linear ICA. Speciﬁc func-
...
O = Ex∼p(x) [J(x) + λ(cid:107)x − V (F (x))(cid:107)2,1] ,

(5)

where (cid:107) · (cid:107)2,1 is the L1 norm and λ is a hyperparameter which controls the trade-off between the
...
Z ← F (X) // apply encoder
ˆX ← V (Z) // apply decoder
ˆZ ← Resample(Z) or sample ˆZ ∼ G(ˆZ)
J = log(D(Z)) + log(1 − D(ˆZ))
Update D to maximize J
R = (cid:107)X − ˆX(cid:107)2,1
...
and from growing indeﬁnitely in magnitude, potentially causing the discriminator to fail because it
...
algorithm (Hyv¨arinen & Oja, 1997), or on minimization of the mutual information of the extracted
...
This work was partially inspired by J¨urgen Schmidhuber’s work on the learning of binary factorial
...
any sampling and a continuous version of it would be an interesting subject for future research.
...
learned features to adhere to its speciﬁc shape and this may be in competition with the independence
...
(St¨ogbauer et al., 2004). Unfortunately, this estimator is biased when variables are far from inde-
...
purposes. The data set consisted of the ﬁrst 4000 samples of these signals.2 For the audio separation
tasks, we used speech recordings from the 16kHz version of the freely available TSP data set (Kabal,
2002). The ﬁrst source was an utterance from a male speaker (MA02 04.wav), the second source
an utterance from a female speaker (FA01 03.wav), and the third source was uniform noise. All
...
Linear ICA As a ﬁrst proof of concept, we trained a model in which both the model and the
...
discriminator network was a multilayer perceptron with one hidden layer of 64 rectiﬁed linear units.
...
the inverse of the mixing transformation, is as ﬂexible as possible, while the decoder is constrained
in the types of functions it can learn. One could also choose a ﬂexible decoder while keeping the
encoder constrained but this didn’t seem to work well in practice. The encoder was a multi-layer
perceptron (MLP) with two hidden layers of rectiﬁed linear units (ReLU; Nair & Hinton 2010). The
ﬁrst layer of the decoder was a linear transformation. Subsequently, each output was processed by
...
010020030040050010101002003004005001010100200300400500101010020030040050010101002003004005001010100200300400500505Under review as a conference paper at ICLR 2018

any sigmoid functions in our model. The discriminator network was again multilayer perceptron
with one hidden layer of 64 rectiﬁed linear units.

Over-determined multi-layer non-linear mixture With this task, we illustrate the beneﬁt of our
...
works. There is no guarantee of identiﬁability for this task, but the large number of observations
makes it more likely.

Baselines For the linear problems, we compared our results with the FastICA (Hyv¨arinen & Oja,
...
model (Almeida, 2003) with a neural network architecture speciﬁcally proposed for these types of
problems (Zheng et al., 2007). We also computed ρmax for the mixed signals. Unfortunately, we
couldn’t ﬁnd a proper baseline for the over-determined MLP problem.
...
For each choice of hyper-parameters, we ran ﬁve experiments with different seeds. After discarding
...
on batches of 64 samples using RMSProp (Tieleman & Hinton, 2012).

The standard JS-divergence optimizing GAN loss was used for all the hyper-parameter tuning exper-
iment. We didn’t ﬁnd it necessary to use the commonly used modiﬁcation of this loss for preventing
...
Table 1: Maximum correlation results on all tasks for the synthetic data. A ‘g’ in the sufﬁx of the
model name indicates that a separate generator network was used instead of the resampling method.
Two scores separated by a ‘/’ indicate that the ﬁrst score was obtained using the model selection
...
task. This indicates that the overcompleteness of the task was in this case sufﬁciently helpful for
...
precision without explicitly using properties which are speciﬁc to the linear problem. On the PNL
...
0500001000001500002000002500000.20.40.60.81.01.21.41.6050000100000150000200000250000012345050000100000150000200000250000weight updates0.000.020.040.060.080.100.12rec. costGANWGAN-GP050000100000150000200000250000weight updates0.700.750.800.850.900.951.00max corr.GANWGAN-GPUnder review as a conference paper at ICLR 2018

Table 2: Maximum correlation results on all tasks for the audio data. A ‘g’ in the sufﬁx of the model
name indicates that a separate generator network was used instead of the resampling method. Two
scores separated by a ‘/’ indicate that the ﬁrst score was obtained using the model selection described
...
A serious difﬁculty with evaluating general methods for learning independent features is that the
...
lection method are ﬁrst steps in the way to evaluating these models but more principled approaches
...
and solve linear and non-linear ICA problems without relying on speciﬁc properties of the mixing
...
Lu´ıs B Almeida. Misep–linear and nonlinear ica based on mutual information. Journal of Machine

Learning Research, 4(Dec):1297–1318, 2003.
...
In Artiﬁcial Intelligence and Statistics, pp. 460–469, 2017.

Aapo Hyv¨arinen and Erkki Oja. One-unit learning rules for independent component analysis. Ad-

vances in neural information processing systems, pp. 480–486, 1997.

Aapo Hyv¨arinen, Juha Karhunen, and Erkki Oja. Independent component analysis, volume 46. John
...
Alexander Kraskov, Harald St¨ogbauer, and Peter Grassberger. Estimating mutual information. Phys-
...
Dmitriy Serdyuk, Kartik Audhkhasi, Phil´emon Brakel, Bhuvana Ramabhadran, Samuel Thomas,
Invariant representations for noisy speech recognition. arXiv preprint

and Yoshua Bengio.
arXiv:1612.01928, 2016.

Harald St¨ogbauer, Alexander Kraskov, Sergey A Astakhov, and Peter Grassberger. Least-dependent-
...
The synthetic signals were deﬁned as follows:
...
s4(t) = sin(180πt),
s5(t) ∼ uniform(x|[−1, 1]),
s6(t) ∼ laplace(x|µ = 0, b = 1).

The experiments were done using the ﬁrst 4000 samples with t linearly spaced between [0, 0.4].
...
01002003004005001010100200300400500101010020030040050010101002003004005001010100200300400500101010020030040050050501002003004005001010100200300400500101010020030040050010101002003004005000.50.00.51.001002003004005001010100200300400500101Under review as a conference paper at ICLR 2018
...
01002003004005000.50.00.51.001002003004005001.00.50.001002003004005000.00.51.001002003004005001.00.50.001002003004005001.51.00.50.00.501002003004005001010100200300400500101010020030040050010101002003004005001010100200300400500101010020030040050010101002003004005000.50.00.51.0